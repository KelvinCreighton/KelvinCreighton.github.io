...
export default function NoSQLInjectionWriteup() {
  ...
  password: password.startsWith("{") && password.endsWith("}") ? JSON.parse(password) : password,
            ...
            reason:
              "<strong>Reason:</strong> The code attempts to parse the `email` and `password` fields as JSON if they start and end with braces. This is a severe vulnerability as it allows us to supply MongoDB query operators (like `$ne`) as the password instead of a string, leading to a NoSQL Injection.",
          },
        ],
      },
      {
        ...
        steps: [
          {
            ...
            reason:
              '<strong>Reason:</strong> The payload `{\\"$ne\\":null}` translates to "not equal to null". Since every user has a password, this condition is true. The database finds the user matching the email and a non-null password, granting us access.',
          },
          {
            number: "02",
