function level5() {
	...
	ctx.stroke();
		...
		//smallBlock2
	var smallBlock2 = {
			x: canvas.width - ball.r2 * 2 - ball.r2 / 2,
			y: canvas.height - ball.r2 * 4 - ball.r2 / 2 - 1,
			w: ball.r2 * 2,
			h: ball.r2 * 2,
			r: ball.r2 + 0.5
		};
	...
	ctx.stroke();
		...
		//box5
	var box5 = {
			x: box3.x + box1.w * 2 - box1.w / 4,
			y: box1.h - box1.h / 4,
			w: box1.w,
			h: box1.h * 6 - box1.h + box1.h / 4
		};
	ctx.beginPath();
	...
	ctx.stroke();
		...
		//box6
	var box6 = {
			x: box5.x + box1.w * 2 - box1.w / 4,
			y: 0,
			w: box1.w,
			h: canvas.height - box1.w - ball.r2 - ball.r2 / 2
		};
	ctx.beginPath();
	...
	var box7 = {
			x: box1.w,
			y: canvas.height - box1.h * 2,
			w: box1.w * 10 + ball.r2 / 2,
			h: ball.r2 / 2
		};
	ctx.beginPath();
	...
	ctx.stroke();
		...
		//box8
	var box8 = {
			x: box7.x + box7.w - box1.w + ball.r2 / 2,
			y: box7.y - box1.h + ball.r2 / 2,
			w: box1.w - ball.r2 / 2,
			h: box1.h - ball.r2 / 2
		};
	ctx.beginPath();
	...
	var box12 = {
			x: box1.x + box1.w * 2,
			y: box1.y + box1.w * 8 + ball.r2 / 4,
			w: box1.w / 2,
			h: box1.h + ball.r2 - ball.r2 / 4
		};
	ctx.beginPath();
	...
	ctx.stroke();
		...
		//box14
	var box14 = {
			x: box5.x + box1.w * 5 - ball.r2 * 1.5,
			y: box6.h / 2 - ball.r2 / 2,
			w: box1.w,
			h: canvas.height - box6.h / 1.5
		};
	ctx.beginPath();
	...
	ctx.stroke();
		...
		//box15
	var box15 = {
			x: box5.x + box1.w * 4 - ball.r2 * 2,
			y: box6.y + box1.h * 2 + ball.r2,
			w: box1.w * 2,
			h: ball.r2
		};
	ctx.beginPath();
	...
	for(spike1i = 0; spike1i < spike1.w / ball.r2; spike1i++) {
		...
		var spike1Dis1 = spike1.x2 - ball.x - ball.r2;
			spike1Dis2 = ball.x - ball.r2 - spike1.x - spike1.w;
			spike1Dis3 = spike1.y - ball.y - ball.r2;
			spike1Dis4 = spike1.y + spike1.h - ball.y + ball.r2;
		
		if(ball.xs1 >= spike1Dis1 && ball.y + ball.r2 > spike1.y && ball.y - ball.r2 < spike1.y + spike1.h2 - 2 && ball.x < spike1.x) {
			start = true;
		}
		if(ball.xs2 >= spike1Dis2 && ball.y + ball.r2 > spike1.y && ball.y - ball.r2 < spike1.y + spike1.h2 - 2 && ball.x > spike1.x) {
			start = true;
		}
		if(ball.g >= spike1Dis3 && ball.y < spike1.y && ball.x + ball.r2 > spike1.x && ball.x - ball.r2 < spike1.x + spike1.w) {
			start = true;
		}
		if(spike1Dis4 >= ball.g && ball.g < 0 && ball.x + ball.r2 > spike1.x && ball.x - ball.r2 < spike1.x + spike1.w && ball.y + ball.r2 > spike1.y + spike1.h) {
			start = true;
		}
		
		//spike2
					//dont use this for copy-paste
	var spike2 = {
			x: box2.x + box2.w,
			x2: box2.x + box2.w,
			y: ball.r2,
	...
	for(spike2i = 0; spike2i < spike2.w / ball.r2; spike2i++) {
		...
		ctx.beginPath();
		ctx.moveTo(spike2disp.spikeX + spike2Size / 2 + 2.5, spike2disp.spikeY);
		ctx.lineTo(spike2disp.spikeX + spike2Size + 5, spike2disp.spikeY - spike2Size);
		ctx.moveTo(spike2disp.spikeX + spike2Size + 5, spike2disp.spikeY - spike2Size);
		ctx.lineTo(spike2disp.spikeX, spike2disp.spikeY - spike2Size);
		ctx.moveTo(spike2disp.spikeX, spike2disp.spikeY - spike2Size);
		ctx.lineTo(spike2disp.spikeX + spike2Size / 2 + 2.5, spike2disp.spikeY);
		ctx.stroke();
	}
		var spike2Dis1 = spike2.x2 - ball.x - ball.r2;
			spike2Dis2 = ball.x - ball.r2 - spike2.x - spike2.w;
			spike2Dis3 = spike2.y - ball.y - ball.r2;
			spike2Dis4 = spike2.y + spike2.h - ball.y + ball.r2;
		
		if(ball.xs1 >= spike2Dis1 && ball.y + ball.r2 > spike2.y && ball.y - ball.r2 < spike2.y + spike2.h2 - 2 && ball.x < spike2.x) {
			start = true;
		}
		if(ball.xs2 >= spike2Dis2 && ball.y + ball.r2 > spike2.y && ball.y - ball.r2 < spike2.y + spike2.h2 - 2 && ball.x > spike2.x) {
			start = true;
		}
		if(ball.g >= spike2Dis3 && ball.y < spike2.y && ball.x + ball.r2 > spike2.x && ball.x - ball.r2 < spike2.x + spike2.w) {
			start = true;
		}
		if(spike2Dis4 >= ball.g && ball.g < 0 && ball.x + ball.r2 > spike2.x && ball.x - ball.r2 < spike2.x + spike2.w && ball.y + ball.r2 > spike2.y + spike2.h) {
			start = true;
		}
	...
	for(spike6i = 0; spike6i < spike6.w / ball.r2; spike6i++) {
		...
		var spike6Dis1 = spike6.x2 - ball.x - ball.r;
			spike6Dis2 = ball.x - ball.r - spike6.x - spike6.w2;
			spike6Dis3 = spike6.y - ball.y - ball.r2;
			spike6Dis4 = ball.y - ball.r - spike6.y - spike6.h;
		
		if(ball.xs1 >= spike6Dis1 && ball.y + ball.r > spike6.y && ball.y - ball.r < spike6.y + spike6.h - 2 && ball.x < spike6.x2) {
			start = true;
		}
		if(ball.xs2 >= spike6Dis2 && ball.y + ball.r > spike6.y && ball.y - ball.r < spike6.y + spike6.h - 2 && ball.x > spike6.x2) {
			start = true;
		}
		if(ball.g >= spike6Dis3 && ball.y < spike6.y && ball.x + ball.r > spike6.x2 && ball.x - ball.r < spike6.x2 + spike6.w2) {
			start = true;
		}
		if(spike6Dis4 <= ball.g && ball.y > spike6.y + spike6.h && ball.g < 0 && ball.x + ball.r > spike6.x2 && ball.x - ball.r < spike6.x2 + spike6.w2) {
			start = true;
		}
		
		//spike7
	var spike7 = {
			x: box7.x + box1.w * 2 + ball.r2,
			x2: box7.x + box1.w * 2 + ball.r2 + ball.r2 / 2,
	...
	for(spike7i = 0; spike7i < spike7.w / ball.r2; spike7i++) {
		...
		var spike7Dis1 = spike7.x2 - ball.x - ball.r2;
			spike7Dis2 = ball.x - ball.r2 - spike7.x2 - spike7.w2;
			spike7Dis3 = spike7.y - ball.y - ball.r2;
			spike7Dis4 = spike7.y + spike7.h - ball.y + ball.r2;
		
		if(ball.xs1 >= spike7Dis1 && ball.y + ball.r2 > spike7.y && ball.y - ball.r2 < spike7.y + spike7.h2 - 2 && ball.x < spike7.x2) {
			start = true;
		}
		if(ball.xs2 >= spike7Dis2 && ball.y + ball.r2 > spike7.y && ball.y - ball.r2 < spike7.y + spike7.h2 - 2 && ball.x > spike7.x2) {
			start = true;
		}
		if(ball.g >= spike7Dis3 && ball.y < spike7.y && ball.x + ball.r2 > spike7.x2 && ball.x - ball.r2 < spike7.x2 + spike7.w2) {
			start = true;
		}
		if(spike7Dis4 >= ball.g && ball.g < 0 && ball.x + ball.r2 > spike7.x2 && ball.x - ball.r2 < spike7.x2 + spike7.w2 && ball.y + ball.r2 > spike7.y + spike7.h) {
			start = true;
		}
		
		//spike8
	var spike8 = {
			x: spike7.x + spike7.w + ball.r2 / 2,
			x2: spike7.x + spike7.w + ball.r2,
			y: canvas.height - ball.r2,
			w: box1.w * 2 + ball.r2,
			w2: box1.w * 2,
			h: ball.r2
	...
	for(spike8i = 0; spike8i < spike8.w / ball.r2; spike8i++) {
		...
		var spike8Dis1 = spike8.x2 - ball.x - ball.r;
			spike8Dis2 = ball.x - ball.r - spike8.x - spike8.w2;
			spike8Dis3 = spike8.y - ball.y - ball.r2;
			spike8Dis4 = ball.y - ball.r - spike8.y - spike8.h;
		
		if(ball.xs1 >= spike8Dis1 && ball.y + ball.r > spike8.y && ball.y - ball.r < spike8.y + spike8.h - 2 && ball.x < spike8.x2) {
			start = true;
		}
		if(ball.xs2 >= spike8Dis2 && ball.y + ball.r > spike8.y && ball.y - ball.r < spike8.y + spike8.h - 2 && ball.x > spike8.x2) {
			start = true;
		}
		if(ball.g >= spike8Dis3 && ball.y < spike8.y && ball.x + ball.r > spike8.x2 && ball.x - ball.r < spike8.x2 + spike8.w2) {
			start = true;
		}
		if(spike8Dis4 <= ball.g && ball.y > spike8.y + spike8.h && ball.g < 0 && ball.x + ball.r > spike8.x2 && ball.x - ball.r < spike8.x2 + spike8.w2) {
			start = true;
		}
	...
	var spike9 = {
			...
			x2: spike8.x + spike8.w + box1.w + ball.r2 / 2,
			y: box7.y + box7.h + ball.r2,
			w: box1.w,
			w2: box1.w - ball.r2,
			h: -ball.r2,
	...
	for(spike9i = 0; spike9i < spike9.w / ball.r2; spike9i++) {
		...
		var spike9Dis1 = spike9.x2 - ball.x - ball.r2;
			spike9Dis2 = ball.x - ball.r2 - spike9.x2 - spike9.w2;
			spike9Dis3 = spike9.y - ball.y - ball.r2;
			spike9Dis4 = spike9.y + spike9.h - ball.y + ball.r2;
		
		if(ball.xs1 >= spike9Dis1 && ball.y + ball.r2 > spike9.y && ball.y - ball.r2 < spike9.y + spike9.h2 - 2 && ball.x < spike9.x2) {
			start = true;
		}
		if(ball.xs2 >= spike9Dis2 && ball.y + ball.r2 > spike9.y && ball.y - ball.r2 < spike9.y + spike9.h2 - 2 && ball.x > spike9.x2) {
			start = true;
		}
		if(ball.g >= spike9Dis3 && ball.y < spike9.y && ball.x + ball.r2 > spike9.x2 && ball.x - ball.r2 < spike9.x2 + spike9.w2) {
			start = true;
		}
		if(spike9Dis4 >= ball.g && ball.g < 0 && ball.x + ball.r2 > spike9.x2 && ball.x - ball.r2 < spike9.x2 + spike9.w2 && ball.y + ball.r2 > spike9.y + spike9.h) {
			start = true;
		}
	...
	var spike10 = {
			x: spike8.x + spike8.w,
			x2: spike8.x + spike8.w + ball.r2 / 2,
			y: canvas.height - ball.r2,
			w: ball.r2,
			w2: 0,
			h: ball.r2
	...
	for(spike10i = 0; spike10i < spike10.w / ball.r2; spike10i++) {
		...
		var spike10Dis1 = spike10.x2 - ball.x - ball.r;
			spike10Dis2 = ball.x - ball.r - spike10.x - spike10.w2;
			spike10Dis3 = spike10.y - ball.y - ball.r2;
			spike10Dis4 = ball.y - ball.r - spike10.y - spike10.h;
		
		if(ball.xs1 >= spike10Dis1 && ball.y + ball.r > spike10.y && ball.y - ball.r < spike10.y + spike10.h - 2 && ball.x < spike10.x2) {
			start = true;
		}
		if(ball.xs2 >= spike10Dis2 && ball.y + ball.r > spike10.y && ball.y - ball.r < spike10.y + spike10.h - 2 && ball.x > spike10.x2) {
			start = true;
		}
		if(ball.g >= spike10Dis3 && ball.y < spike10.y && ball.x + ball.r > spike10.x2 && ball.x - ball.r < spike10.x2 + spike10.w2) {
			start = true;
		}
		if(spike10Dis4 <= ball.g && ball.y > spike10.y + spike10.h && ball.g < 0 && ball.x + ball.r > spike10.x2 && ball.x - ball.r < spike10.x2 + spike10.w2) {
			start = true;
		}
	...
	for(spike11i = 0; spike11i < spike11.w / ball.r2; spike11i++) {
		...
		var spike11Dis1 = spike11.x2 - ball.x - ball.r;
			spike11Dis2 = ball.x - ball.r - spike11.x - spike11.w2;
			spike11Dis3 = spike11.y - ball.y - ball.r2;
			spike11Dis4 = ball.y - ball.r - spike11.y - spike11.h;
		
		if(ball.xs1 >= spike11Dis1 && ball.y + ball.r > spike11.y && ball.y - ball.r < spike11.y + spike11.h - 2 && ball.x < spike11.x2) {
			start = true;
		}
		if(ball.xs2 >= spike11Dis2 && ball.y + ball.r > spike11.y && ball.y - ball.r < spike11.y + spike11.h - 2 && ball.x > spike11.x2) {
			start = true;
		}
		if(ball.g >= spike11Dis3 && ball.y < spike11.y && ball.x + ball.r > spike11.x2 && ball.x - ball.r < spike11.x2 + spike11.w2) {
			start = true;
		}
		if(spike11Dis4 <= ball.g && ball.y > spike11.y + spike11.h && ball.g < 0 && ball.x + ball.r > spike11.x2 && ball.x - ball.r < spike11.x2 + spike11.w2) {
			start = true;
		}
	...
	for(spike12i = 0; spike12i < spike12.w / ball.r2; spike12i++) {
		...
		var spike12Dis1 = spike12.x2 - ball.x - ball.r2;
			spike12Dis2 = ball.x - ball.r2 - spike12.x2 - spike12.w2;
			spike12Dis3 = spike12.y - ball.y - ball.r2;
			spike12Dis4 = spike12.y + spike12.h - ball.y + ball.r2;
		
		if(ball.xs1 >= spike12Dis1 && ball.y + ball.r2 > spike12.y && ball.y - ball.r2 < spike12.y + spike12.h2 - 2 && ball.x < spike12.x2) {
			start = true;
		}
		if(ball.xs2 >= spike12Dis2 && ball.y + ball.r2 > spike12.y && ball.y - ball.r2 < spike12.y + spike12.h2 - 2 && ball.x > spike12.x2) {
			start = true;
		}
		if(ball.g >= spike12Dis3 && ball.y < spike12.y && ball.x + ball.r2 > spike12.x2 && ball.x - ball.r2 < spike12.x2 + spike12.w2) {
			start = true;
		}
		if(spike12Dis4 >= ball.g && ball.g < 0 && ball.x + ball.r2 > spike12.x2 && ball.x - ball.r2 < spike12.x2 + spike12.w2 && ball.y + ball.r2 > spike12.y + spike12.h) {
			start = true;
		}
	...
	var spike13array = [];
	var spike13move;
	var spike13Size = ball.r2;
	
	for(spike13move = 0; spike13move < spike13.w / ball.r2; spike13move++) {
		spike13array.push({
			spikeY: spike13.y,
			spikeX: spike13.x + spike13move * spike13Size	
		});
	}
	var spike13i, spike13disp
	for(spike13i = 0; spike13i < spike13.w / ball.r2; spike13i++) {
		spike13disp = spike13array[spike13i];
		
		ctx.beginPath();
		ctx.moveTo(spike13disp.spikeX + spike13Size / 2, spike13disp.spikeY);
		ctx.lineTo(spike13disp.spikeX + spike13Size, spike13disp.spikeY + spike13Size);
		ctx.moveTo(spike13disp.spikeX + spike13Size, spike13disp.spikeY + spike13Size);
		ctx.lineTo(spike13disp.spikeX, spike13disp.spikeY + spike13Size);
		ctx.moveTo(spike13disp.spikeX, spike13disp.spikeY + spike13Size);
		ctx.lineTo(spike13disp.spikeX + spike13Size / 2, spike13disp.spikeY);
		ctx.stroke();
	}
		var spike13Dis1 = spike13.x2 - ball.x - ball.r;
			spike13Dis2 = ball.x - ball.r - spike13.x - spike13.w2;
			spike13Dis3 = spike13.y - ball.y - ball.r2;
			spike13Dis4 = ball.y - ball.r - spike13.y - spike13.h;
		
		if(ball.xs1 >= spike13Dis1 && ball.y + ball.r > spike13.y && ball.y - ball.r < spike13.y + spike13.h - 2 && ball.x < spike13.x2) {
			start = true;
		}
		if(ball.xs2 >= spike13Dis2 && ball.y + ball.r > spike13.y && ball.y - ball.r < spike13.y + spike13.h - 2 && ball.x > spike13.x2) {
			start = true;
		}
		if(ball.g >= spike13Dis3 && ball.y < spike13.y && ball.x + ball.r > spike13.x2 && ball.x - ball.r < spike13.x2 + spike13.w2) {
			start = true;
		}
		if(spike13Dis4 <= ball.g && ball.y > spike13.y + spike13.h && ball.g < 0 && ball.x + ball.r > spike13.x2 && ball.x - ball.r < spike13.x2 + spike13.w2) {
			start = true;
		}
	...
	if(ball.gAcc == 0.04 || ball.gAcc == -0.04) {
		ctx.beginPath();
		ctx.rect(halfBlock2.x, halfBlock2.y, halfBlock2.w, halfBlock2.h);
		ctx.stroke();
		ctx.beginPath();
		ctx.rect(halfBlock2.x + halfBlock2.w / 4, halfBlock2.y + halfBlock2.h / 4, halfBlock2.w / 2, halfBlock2.h / 2);
		ctx.stroke();
			
		var halfBlock2Dis1 = halfBlock2.x - ball.x - ball.r;
			halfBlock2Dis2 = ball.x - ball.r - halfBlock2.x - halfBlock2.w;
			halfBlock2Dis3 = halfBlock2.y - ball.y - ball.r;
			halfBlock2Dis4 = ball.y - ball.r - halfBlock2.y - halfBlock2.h;
		
		if(ball.xs1 >= halfBlock2Dis1 && ball.y + ball.r > halfBlock2.y && ball.y - ball.r < halfBlock2.y + halfBlock2.h  - 2 && ball.x < halfBlock2.x) {
			if(ball.gAcc > 0) {
				ball.gAcc = 0.02;
			} else if(ball.gAcc < 0) {
				ball.gAcc = -0.02;
		...
		if(ball.xs2 >= halfBlock2Dis2 && ball.y + ball.r > halfBlock2.y && ball.y - ball.r < halfBlock2.y + halfBlock2.h  - 2 && ball.x > halfBlock2.x) {
			if(ball.gAcc > 0) {
				ball.gAcc = 0.02;
			} else if(ball.gAcc < 0) {
				ball.gAcc = -0.02;
		...
		if(ball.g >= halfBlock2Dis3 && ball.y < halfBlock2.y && ball.x + ball.r > halfBlock2.x && ball.x - ball.r < halfBlock2.x + halfBlock2.w) {
			if(ball.gAcc > 0) {
				ball.gAcc = 0.02;
			} else if(ball.gAcc < 0) {
				ball.gAcc = -0.02;
		...
		if(halfBlock2Dis4 <= ball.g && ball.y > halfBlock2.y + halfBlock2.h && ball.g < 0 && ball.x + ball.r > halfBlock2.x && ball.x - ball.r < halfBlock2.x + halfBlock2.w) {
			if(ball.gAcc > 0) {
				ball.gAcc = 0.02;
			} else if(ball.gAcc < 0) {
				ball.gAcc = -0.02;
	...
	var reverseBlock4 = {
		...
		var reverseBlock4Dis1 = reverseBlock4.x - ball.x - ball.r;
			reverseBlock4Dis2 = ball.x - ball.r - reverseBlock4.x - reverseBlock4.w;
			reverseBlock4Dis3 = reverseBlock4.y - ball.y - ball.r;
			reverseBlock4Dis4 = ball.y - ball.r - reverseBlock4.y - reverseBlock4.h;

	if(gFlipper == false) {
		ctx.beginPath();
		ctx.rect(reverseBlock4.x2, reverseBlock4.y2, reverseBlock4.w2, reverseBlock4.h2);
		ctx.stroke();
		ctx.beginPath();
		ctx.arc(reverseBlock4.x2 + reverseBlock4.w2 / 2, reverseBlock4.y2 + reverseBlock4.h2 / 2, reverseBlock4.r, 0, Math.PI * 2);
		ctx.stroke();
		
		if(ball.xs1 >= reverseBlock4Dis1 && ball.y + ball.r > reverseBlock4.y && ball.y - ball.r < reverseBlock4.y + reverseBlock4.h  - 2 && ball.x < reverseBlock4.x) {
			gFlipper = true;
		}
		if(ball.xs2 >= reverseBlock4Dis2 && ball.y + ball.r > reverseBlock4.y && ball.y - ball.r < reverseBlock4.y + reverseBlock4.h  - 2 && ball.x > reverseBlock4.x) {
			gFlipper = true;
		}
		if(ball.g >= reverseBlock4Dis3 && ball.y < reverseBlock4.y && ball.x + ball.r > reverseBlock4.x && ball.x - ball.r < reverseBlock4.x + reverseBlock4.w) {
			gFlipper = true;
		}
		if(reverseBlock4Dis4 <= ball.g && ball.y > reverseBlock4.y + reverseBlock4.h && ball.g < 0 && ball.x + ball.r > reverseBlock4.x && ball.x - ball.r < reverseBlock4.x + reverseBlock4.w) {
			gFlipper = true;
		}
	}
	...
	if(gFlipper == true) {
		ctx.beginPath();
		ctx.rect(notreverseBlock3.x, notreverseBlock3.y, notreverseBlock3.w, notreverseBlock3.h);
		ctx.stroke();
		ctx.beginPath();
		ctx.arc(notreverseBlock3.x + notreverseBlock3.w / 2, notreverseBlock3.y + notreverseBlock3.h / 2, notreverseBlock3.r, 0, Math.PI * 2);
		ctx.stroke();
	...
	//notreverseBlock4
	var notreverseBlock4 = {
			x: box23.x,
			y: box23.y - ball.r2 - ball.r2 / 2,
			w: ball.r2,
			h: ball.r2,
			r: ball.r2 / 4
			};
			
		var notreverseBlock4Dis1 = notreverseBlock4.x - ball.x - ball.r;
			notreverseBlock4Dis2 = ball.x - ball.r - notreverseBlock4.x - notreverseBlock4.w;
			notreverseBlock4Dis3 = notreverseBlock4.y - ball.y - ball.r;
			notreverseBlock4Dis4 = ball.y - ball.r - notreverseBlock4.y - notreverseBlock4.h;

	if(gFlipper == true) {
		ctx.beginPath();
		ctx.rect(notreverseBlock4.x, notreverseBlock4.y, notreverseBlock4.w, notreverseBlock4.h);
		ctx.stroke();
		ctx.beginPath();
		ctx.arc(notreverseBlock4.x + notreverseBlock4.w / 2, notreverseBlock4.y + notreverseBlock4.h / 2, notreverseBlock4.r, 0, Math.PI * 2);
		ctx.stroke();
		
		if(ball.xs1 >= notreverseBlock4Dis1 && ball.y + ball.r > notreverseBlock4.y && ball.y - ball.r < notreverseBlock4.y + notreverseBlock4.h  - 2 && ball.x < notreverseBlock4.x) {
			gFlipper = false;
		}
		if(ball.xs2 >= notreverseBlock4Dis2 && ball.y + ball.r > notreverseBlock4.y && ball.y - ball.r < notreverseBlock4.y + notreverseBlock4.h  - 2 && ball.x > notreverseBlock4.x) {
			gFlipper = false;
		}
		if(ball.g >= notreverseBlock4Dis3 && ball.y < notreverseBlock4.y && ball.x + ball.r > notreverseBlock4.x && ball.x - ball.r < notreverseBlock4.x + notreverseBlock4.w) {
			gFlipper = false;
		}
		if(notreverseBlock4Dis4 <= ball.g && ball.y > notreverseBlock4.y + notreverseBlock4.h && ball.g < 0 && ball.x + ball.r > notreverseBlock4.x && ball.x - ball.r < notreverseBlock4.x + notreverseBlock4.w) {
			gFlipper = false;
		}
	}
	...
	var checkpoint3 = {
			x: box14.x - box1.w + ball.r2 / 4,
			y: box8.y - ball.r2 - ball.r2 / 2,
			w: ball.r2 / 2,
			h: ball.r2 / 2
		};
	ctx.beginPath();
	ctx.rect(checkpoint3.x, checkpoint3.y, checkpoint3.w, checkpoint3.h);
	ctx.stroke();
			
		var checkpoint3Dis1 = checkpoint3.x - ball.x - ball.r;
			checkpoint3Dis2 = ball.x - ball.r - checkpoint3.x - checkpoint3.w;
			checkpoint3Dis3 = checkpoint3.y - ball.y - ball.r;
			checkpoint3Dis4 = checkpoint3.y + checkpoint3.h - ball.y + ball.r;
				//left side
		if(ball.xs1 >= checkpoint3Dis1 && ball.y + ball.r > checkpoint3.y && ball.y - ball.r < checkpoint3.y + checkpoint3.h  - 2 && ball.x < checkpoint3.x) {
			checkpoint = 3;
		}		//right side
		if(ball.xs2 >= checkpoint3Dis2 && ball.y + ball.r > checkpoint3.y && ball.y - ball.r < checkpoint3.y + checkpoint3.h  - 2 && ball.x > checkpoint3.x) {
			checkpoint = 3;
		}		//top side
		if(ball.g >= checkpoint3Dis3 && ball.y < checkpoint3.y && ball.x + ball.r > checkpoint3.x && ball.x - ball.r < checkpoint3.x + checkpoint3.w) {
			checkpoint = 3;
		}		//bottom side
		if(checkpoint3Dis4 >= ball.g && ball.g < 0 && ball.x + ball.r > checkpoint3.x && ball.x - ball.r < checkpoint3.x + checkpoint3.w && ball.y + ball.r > checkpoint3.y + checkpoint3.h) {
			checkpoint = 3;
		}
	}
}